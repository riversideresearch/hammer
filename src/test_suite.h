...
#define g_check_inttype(fmt, typ, n1, op, n2)                                                      \
    do {                                                                                           \
        ...
        typ _n2 = (n2);                                                                            \
        if (!(_n1 op _n2)) {                                                                       \
            g_test_message("Check failed: (%s): (" fmt " %s " fmt ")", #n1 " " #op " " #n2, _n1,   \
                           #op, _n2);                                                              \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)
...
#define g_check_bytes(len, n1, op, n2)                                                             \
    do {                                                                                           \
        ...
        if (!(memcmp(_n1, _n2, len) op 0)) {                                                       \
            g_test_message("Check failed: (%s)", #n1 " " #op " " #n2);                             \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)
...
#define g_check_string(n1, op, n2)                                                                 \
    do {                                                                                           \
        ...
        if (!(strcmp(_n1, _n2) op 0)) {                                                            \
            g_test_message("Check failed: (%s) (%s %s %s)", #n1 " " #op " " #n2, _n1, #op, _n2);   \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)

#define g_check_maybe_string_eq(n1, n2)                                                            \
    do {                                                                                           \
        const char *_n1 = (n1);                                                                    \
        const char *_n2 = (n2);                                                                    \
        if (_n1 != _n2 && _n1 != NULL && _n2 != NULL) {                                            \
            if (!(strcmp(_n1, _n2) == 0)) {                                                        \
                g_test_message("Check failed: (%s) (\"%s\" == \"%s\")", #n1 " == " #n2, _n1, _n2); \
                g_test_fail();                                                                     \
            }                                                                                      \
        } else {                                                                                   \
            if (_n1 != NULL || _n2 != NULL) {                                                      \
                if (_n1 != NULL && _n2 == NULL) {                                                  \
                    g_test_message("Check failed: (%s) (\"%s\" == NULL)", #n1 " == " #n2, _n1);    \
                    g_test_fail();                                                                 \
                } else if (_n1 == NULL && _n2 != NULL) {                                           \
                    g_test_message("Check failed: (%s) (NULL == \"%s\")", #n1 " == " #n2, _n2);    \
                    g_test_fail();                                                                 \
                }                                                                                  \
...
#define g_check_contextfree(lang)                                                                  \
    do {                                                                                           \
...
#define g_check_compilable(lang, backend, params)                                                  \
    do {                                                                                           \
...
#define g_check_parse_failed__m(mm__, parser, backend, input, inp_len)                             \
    do {                                                                                           \
        ...
        if (NULL != res) {                                                                         \
            ...
            g_test_message("Check failed: parse shouldn't have succeeded, but did on line %d",     \
                           __LINE__);                                                              \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)
...
#define g_check_parse_failed_no_compile__m(mm__, parser, input, inp_len)                           \
    do {                                                                                           \
        HParseResult *res = h_parse__m(mm__, parser, (const uint8_t *)input, inp_len);             \
        if (NULL != res) {                                                                         \
            print_arena_stats(res->arena);                                                         \
            h_parse_result_free(res);                                                              \
            g_test_message("Check failed: shouldn't have succeeded, but did on ;ine %d",           \
                           __LINE__);                                                              \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)
...
#define g_check_parse_ok__m(mm__, parser, backend, input, inp_len)                                 \
    do {                                                                                           \
        int skip = h_compile__m(mm__, (HParser *)(parser), (HParserBackend)backend, NULL);         \
        if (skip) {                                                                                \
...
#define g_check_parse_ok_no_compile__m(mm__, parser, input, inp_len)                               \
    do {                                                                                           \
        HParseResult *res = h_parse__m(mm__, parser, (const uint8_t *)input, inp_len);             \
        if (!res) {                                                                                \
            g_test_message("Parse failed on line %d", __LINE__);                                   \
...
#define g_check_parse_match__m(mm__, parser, backend, input, inp_len, result)                      \
    do {                                                                                           \
        int skip = h_compile__m(mm__, (HParser *)(parser), (HParserBackend)backend, NULL);         \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
        ...
        HParseResult *res =                                                                        \
            h_parse__m(mm__, (HParser *)(parser), (const uint8_t *)input, inp_len);                \
        if (!res) {                                                                                \
            g_test_message("Parse failed on line %d", __LINE__);                                   \
            g_test_fail();                                                                         \
        } else {                                                                                   \
...
#define g_check_parse_match_no_compile__m(mm__, parser, input, inp_len, result)                    \
    do {                                                                                           \
        HParseResult *res =                                                                        \
            h_parse__m(mm__, (HParser *)(parser), (const uint8_t *)input, inp_len);                \
        if (!res) {                                                                                \
...
#define g_check_parse_chunks_failed__m(mm__, parser, backend, chunk1, c1_len, chunk2, c2_len)      \
    do {                                                                                           \
        int skip = h_compile__m(mm__, (HParser *)(parser), (HParserBackend)backend, NULL);         \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
...
#define g_check_parse_chunks_failed___m(mm__, parser, chunk1, c1_len, chunk2, c2_len)              \
    do {                                                                                           \
...
#define g_check_parse_chunks_ok(parser, backend, chunk1, c1_len, chunk2, c2_len)                   \
    do {                                                                                           \
...
#define g_check_parse_chunks_ok_(parser, chunk1, c1_len, chunk2, c2_len)                           \
    do {                                                                                           \
...
#define g_check_parse_chunks_match(parser, backend, chunk1, c1_len, chunk2, c2_len, result)        \
    do {                                                                                           \
        int skip = h_compile((HParser *)(parser), (HParserBackend)backend, NULL);                  \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
...
#define g_check_parse_chunks_match_(parser, chunk1, c1_len, chunk2, c2_len, result)                \
    do {                                                                                           \
        HSuspendedParser *s = h_parse_start((HParser *)(parser));                                  \
        if (!s) {                                                                                  \
            g_test_message("Chunk-wise parsing not available");                                    \
...
#define g_check_parse_chunk_failed__m(mm__, parser, backend, chunk1, c1_len)                       \
    do {                                                                                           \
        int skip = h_compile__m(mm__, (HParser *)(parser), (HParserBackend)backend, NULL);         \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
...
#define g_check_parse_chunk_failed___m(mm__, parser, chunk1, c1_len)                               \
    do {                                                                                           \
        HSuspendedParser *s = h_parse_start__m(mm__, (HParser *)(parser));                         \
        if (!s) {                                                                                  \
            g_test_message("Chunk-wise parsing not available");                                    \
...
#define g_check_parse_chunk_ok(parser, backend, chunk1, c1_len)                                    \
    do {                                                                                           \
        int skip = h_compile((HParser *)(parser), (HParserBackend)backend, NULL);                  \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
...
#define g_check_parse_chunk_ok_(parser, chunk1, c1_len)                                            \
    do {                                                                                           \
        HSuspendedParser *s = h_parse_start((HParser *)(parser));                                  \
        if (!s) {                                                                                  \
            g_test_message("Chunk-wise parsing not available");                                    \
...
#define g_check_parse_chunk_match(parser, backend, chunk1, c1_len, result)                         \
    do {                                                                                           \
        int skip = h_compile((HParser *)(parser), (HParserBackend)backend, NULL);                  \
        if (skip) {                                                                                \
            g_test_message("Compile failed");                                                      \
...
#define g_check_parse_chunk_match_(parser, chunk1, c1_len, result)                                 \
    do {                                                                                           \
        HSuspendedParser *s = h_parse_start((HParser *)(parser));                                  \
        if (!s) {                                                                                  \
            g_test_message("Chunk-wise parsing not available");                                    \
...
#define g_check_hashtable_present(table, key)                                                      \
    do {                                                                                           \
...
#define g_check_hashtable_absent(table, key)                                                       \
    do {                                                                                           \
        if (h_hashtable_present(table, key)) {                                                     \
...
#define g_check_hashtable_size(table, n)                                                           \
    do {                                                                                           \
        size_t expected = n;                                                                       \
        size_t actual = (table)->used;                                                             \
        if (actual != expected) {                                                                  \
            g_test_message("Check failed: table size should have been %zu, but was %zu", expected, \
                           actual);                                                                \
            g_test_fail();                                                                         \
        }                                                                                          \
    } while (0)

#define g_check_stringmap_present(table, key)                                                      \
    do {                                                                                           \
...
#define g_check_stringmap_absent(table, key)                                                       \
    do {                                                                                           \
        bool end = (key[strlen(key) - 1] == '$');                                                  \
        if (h_stringmap_present(table, (uint8_t *)key, strlen(key), end)) {                        \
