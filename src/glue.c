...
HParsedToken *h_act_index(int i, const HParseResult *p, void *user_data) {
    ...
    const HCountedArray *seq = tok->swig_union.seq;
    size_t n = seq->used;

    if (i < 0 || (size_t)i >= n)
        return NULL;
    else
        return tok->swig_union.seq->elements[i];
}

HParsedToken *h_act_first(const HParseResult *p, void *user_data) {
    assert(p->ast);
    assert(p->ast->token_type == TT_SEQUENCE);
    assert(p->ast->swig_union.seq->used > 0);

    return p->ast->swig_union.seq->elements[0];
}
...
HParsedToken *h_act_second(const HParseResult *p, void *user_data) {
    ...
    return p->ast->swig_union.seq->elements[1];
}
...
HParsedToken *h_act_last(const HParseResult *p, void *user_data) {
    ...
    return p->ast->swig_union.seq->elements[p->ast->swig_union.seq->used - 1];
}
...
static void act_flatten_(HCountedArray *seq, const HParsedToken *tok) {
    if (tok == NULL) {
        ...
        size_t i;
        for (i = 0; i < tok->swig_union.seq->used; i++)
            act_flatten_(seq, tok->swig_union.seq->elements[i]);
    } else {
...
HParsedToken *h_act_flatten(const HParseResult *p, void *user_data) {
    ...
    act_flatten_(res->swig_union.seq, p->ast);
    return res;
}
...
void h_seq_snoc(HParsedToken *xs, const HParsedToken *x) {
    ...
    h_carray_append(xs->swig_union.seq, (HParsedToken *)x);
}
...
void h_seq_append(HParsedToken *xs, const HParsedToken *ys) {
    ...
    for (size_t i = 0; i < ys->swig_union.seq->used; i++)
        h_carray_append(xs->swig_union.seq, ys->swig_union.seq->elements[i]);
}
...
const HParsedToken *h_seq_flatten(HArena *arena, const HParsedToken *p) {
    ...
    case TT_SEQUENCE:
        ...
        for (size_t i = 0; i < p->swig_union.seq->used; i++) {
