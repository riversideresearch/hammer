...
#define NEW_INSTANCE_(env, class, inner)                                                           \
    (*env)->NewObject(env, class, REFCONSTRUCTOR_(env, class), (jlong)inner)

// Safer versions, assert that the result is not NULL
// If one of those asserts fails, it most likely means that there's a typo (wrong class name or
// method signature) or big trouble (OOM)
#define FIND_CLASS(target, env, class)                                                             \
    target = FIND_CLASS_(env, class);                                                              \
    assert(target != NULL)
#define REFCONSTRUCTOR(target, env, class)                                                         \
    target = REFCONSTRUCTOR_(env, class);                                                          \
    assert(target != NULL)
#define NEW_INSTANCE(target, env, class, inner)                                                    \
    target = NEW_INSTANCE_(env, class, inner);                                                     \
    assert(target != NULL)
...
#define RETURNWRAP(env, inner)                                                                     \
    jclass __cls = FIND_CLASS_(env, PARSER_CLASS);                                                 \
    assert(__cls != NULL);                                                                         \
