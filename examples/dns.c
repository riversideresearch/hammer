...
void set_rdata(struct dns_rr *rr, HCountedArray *rdata) {
    ...
    switch (rr->type) {
    case 1:
        rr->a = H_CAST_UINT(p->ast);
        break;
    case 2:
        rr->ns = *H_CAST(dns_domain_t, p->ast);
        break;
    case 3:
        rr->md = *H_CAST(dns_domain_t, p->ast);
        break;
    case 4:
        rr->md = *H_CAST(dns_domain_t, p->ast);
        break;
    case 5:
        rr->cname = *H_CAST(dns_domain_t, p->ast);
        break;
    case 6:
        rr->soa = *H_CAST(dns_rr_soa_t, p->ast);
        break;
    case 7:
        rr->mb = *H_CAST(dns_domain_t, p->ast);
        break;
    case 8:
        rr->mg = *H_CAST(dns_domain_t, p->ast);
        break;
    case 9:
        rr->mr = *H_CAST(dns_domain_t, p->ast);
        break;
    case 10:
        rr->null = *H_CAST(dns_rr_null_t, p->ast);
        break;
    case 11:
        rr->wks = *H_CAST(dns_rr_wks_t, p->ast);
        break;
    case 12:
        rr->ptr = *H_CAST(dns_domain_t, p->ast);
        break;
    case 13:
        rr->hinfo = *H_CAST(dns_rr_hinfo_t, p->ast);
        break;
    case 14:
        rr->minfo = *H_CAST(dns_rr_minfo_t, p->ast);
        break;
    case 15:
        rr->mx = *H_CAST(dns_rr_mx_t, p->ast);
        break;
    case 16:
        rr->txt = *H_CAST(dns_rr_txt_t, p->ast);
        break;
    default:
        break;
    }
...
HParsedToken *act_header(const HParseResult *p, void *user_data) {
    ...
    dns_header_t header_ = {.id = H_CAST_UINT(fields[0]),
...
HParsedToken *act_message(const HParseResult *p, void *user_data) {
    ...
    struct dns_question *questions =
        h_arena_malloc(p->arena, sizeof(struct dns_question) * (header->question_count));
    ...
    struct dns_rr *answers =
        h_arena_malloc(p->arena, sizeof(struct dns_rr) * (header->answer_count));
    ...
    struct dns_rr *authority =
        h_arena_malloc(p->arena, sizeof(struct dns_rr) * (header->authority_count));
    ...
    struct dns_rr *additional =
        h_arena_malloc(p->arena, sizeof(struct dns_rr) * (header->additional_count));
    for (size_t i = 0, j = header->answer_count + header->authority_count;
         i < header->additional_count; ++i, ++j) {
...
const HParser *init_parser() {
    ...
    H_RULE(qtype, h_choice(type, h_int_range(h_uint16(), 252, 255), NULL));
    H_RULE(class, h_int_range(h_uint16(), 1, 4));
    H_RULE(qclass, h_choice(class, h_int_range(h_uint16(), 255, 255), NULL));
    H_RULE(len, h_int_range(h_uint8(), 1, 255));
    H_ARULE(label, h_length_value(len, h_uint8()));
    H_ARULE(qname, h_sequence(h_many1(label), h_ch('\x00'), NULL));
    ...
    H_ARULE(rr, h_sequence(domain,     // NAME
                           ...
                           NULL));
    H_AVRULE(message, h_sequence(header, h_many(question), h_many(rr), h_end_p(), NULL));
...
static const char *TYPE_STR[17] = {"nil", "A",    "NS",  "MD",  "MF",    "CNAME", "SOA", "MB", "MG",
                                   "MR",  "NULL", "WKS", "PTR", "HINFO", "MINFO", "MX",  "TXT"};

const int CLASS_MAX = 4;
const char *CLASS_STR[5] = {"nil", "IN", "CS", "CH", "HS"};
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        packet_size =
            recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr *)&remote, &remote_len);
        // dump the packet...
        ...
        for (size_t i = 0; i < message->header.question_count; i++) {
            ...
            printf("Recieved %s %s request for ", CLASS_STR[question->qclass],
                   TYPE_STR[question->qtype]);
        ...
        // Not much time to actually implement the DNS server for the talk, so here's something
        // quick and dirty. Traditional response for this time of year...
