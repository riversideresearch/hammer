...
// Helper functions for bind parser tests
...
static HParser *bind_cont_returns_null(HAllocator *mm__, const HParsedToken *token, void *user_data) {
    (void)mm__;
    (void)token;
    (void)user_data;
    return NULL;
...
// Continuation that uses realloc to trigger aa_realloc
static HParser *bind_cont_uses_realloc(HAllocator *mm__, const HParsedToken *token, void *user_data) {
    (void)user_data;
    (void)token;
...
// Continuation that uses free to trigger aa_free
...
// Helper functions for attr_bool parser tests
...
static void test_butnot_edge_cases(gconstpointer backend) {
    ...
    const HParser *p4 = h_butnot(h_token((const uint8_t *)"abc", 3), h_token((const uint8_t *)"a", 1));
    ...
    const HParser *p5 = h_butnot(h_token((const uint8_t *)"hello", 5), h_token((const uint8_t *)"he", 2));
    ...
    const HParser *p6 = h_butnot(h_token((const uint8_t *)"abc", 3), h_token((const uint8_t *)"xyz", 3));
    ...
    // When we provide "ab" as first chunk (not last), p1 matches, but p2 needs "abc" and only "ab" available
    // p2 will partially match "ab" then try to read 'c' and overrun -> overrun=true, last_chunk=false
    // This should trigger want_suspend -> return NULL (line 23)
    const HParser *p7 = h_butnot(h_token((const uint8_t *)"ab", 2), h_token((const uint8_t *)"abc", 3));
    ...
    // Second chunk: "c" - would complete p2, but parse should have already failed due to want_suspend
...
static void test_difference_edge_cases(gconstpointer backend) {
    ...
    const HParser *p3 = h_difference(h_token((const uint8_t *)"ab", 2), h_token((const uint8_t *)"abc", 3));
    // First chunk: "ab" (not last) - p1 matches "ab", p2 needs "abc" but only "ab" available
    // p2 will partially match "ab", then try to read 'c' and overrun
    // -> overrun=true, last_chunk=false -> want_suspend returns true -> return NULL (line 23)
    g_check_parse_chunks_failed(p3, be, "ab", 2, "c", 1);
    
    // Test when p1 matches shorter than p2 - should fail (covers line 34: return NULL when r1len < r2len)
    // p1 matches "a" (1 char), p2 matches "ab" (2 chars)
    // But wait - if input is "a", p2 won't match "ab", so r2 will be NULL and it will return r1
    // We need input where both match but p1 is shorter. Let's use "ab" as input:
    // p1 matches "a" (prefix, 1 char), p2 matches "ab" (full match, 2 chars)
    // Actually, h_token matches exactly, so p1 won't match "ab" if it's looking for "a"
    // Let's use a different approach: p1 matches a prefix, p2 matches the full string
    // Actually, let's use h_ch for p1 and h_token for p2
    // Input "ab": p1 (h_ch('a')) matches 'a' (1 char), p2 (h_token("ab")) matches "ab" (2 chars)
    const HParser *p4 = h_difference(h_ch('a'), h_token((const uint8_t *)"ab", 2));
    // Input "ab": p1 matches 'a' (consumes 1 char, r1len=1), p2 matches "ab" (consumes 2 chars, r2len=2)
    // So r1len < r2len -> return NULL (line 34)
...
// Test action parser internal functions for coverage
...
// Test and parser internal functions for coverage
...
// Test choice internal functions and __m variants
static void test_choice_internal(gconstpointer backend) {
    ...
    bool is_valid_regular_false = mixed_choice_regular->vtable->isValidRegular(mixed_choice_regular->env);
    ...
    bool is_valid_regular_false2 = mixed_choice_regular2->vtable->isValidRegular(mixed_choice_regular2->env);
    ...
    bool is_valid_regular_false3 = mixed_choice_regular3->vtable->isValidRegular(mixed_choice_regular3->env);
...
// Test ignore internal functions
static void test_ignore_internal(gconstpointer backend) {
    ...
    bool is_valid_regular_false = ignore_non_regular->vtable->isValidRegular(ignore_non_regular->env);
    g_check_cmp_int(is_valid_regular_false, ==, false);
...
// Test ignoreseq __m functions and desugar branches
static void test_ignoreseq_internal(gconstpointer backend) {
    ...
    const HParser **new_parsers = h_realloc(&system_allocator, test_seq->parsers, sizeof(const HParser *) * 4);
...
// Test indirect internal functions
...
// Test many internal functions for coverage
...
// Test not internal functions for coverage
...
// Test optional internal functions for coverage
